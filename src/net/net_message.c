...
void Net_WriteDeltaPlayerState(mem_buf_t *msg, const player_state_t *from, const player_state_t *to) {
  ...
  if (memcmp(&to->pm_state.params.water_gravity, &from->pm_state.params.water_gravity,
             sizeof(pm_params_t) - offsetof(pm_params_t, water_gravity)) != 0) {
    bits |= PS_PM_PARAMS;
  }
  ...
  if (bits & PS_PM_PARAMS) {
    Net_WriteFloat(msg, to->pm_state.params.water_gravity);
...
void Net_ReadDeltaPlayerState(mem_buf_t *msg, const player_state_t *from, player_state_t *to) {
  ...
  *to = *from;
  ...
  if (bits & PS_PM_PARAMS) {
    to->pm_state.params.water_gravity = Net_ReadFloat(msg);
