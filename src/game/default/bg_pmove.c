...
static void Pm_Gravity(void) {
  ...
  if (pm->water_level > WATER_WAIST) {
    gravity *= pm->s.params.water_gravity;
  }
  ...
  if (pm->s.type == PM_DEAD) {
    ...
    if (pm->s.flags & PMF_DUCKED) { // ducked, reduce height
      ...
      if (pm->s.view_offset.z > target) { // go down
        pm->s.view_offset.z -= pm_locals.time * pm->s.params.speed_duck_stand;
      }
...
static void Pm_AirMove(void) {
  ...
  float max_speed = pm->s.params.speed_air;

  if (max_speed < 1.f) {
    max_speed = 1.f; // air_speed must stay positive to bound the wish-speed
  }
  ...
  float accel = pm->s.params.accel_air;
